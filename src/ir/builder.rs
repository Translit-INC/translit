use crate::{
    Arg, Function, FunctionID, Instruction, InstructionCode as IC, Label,
...
impl IRBuilder {
    ...
    pub fn push(&mut self, code: IC, args: Vec<Arg>) -> TranslitResult<()> {
        match self.functions.last() {
    ...
    pub fn create_var<'a>(&'a mut self, var_value: Literal) -> &'a mut Variable {
        let current_idx = self.memory.len();
        let new_var = Variable(var_value, current_idx);

        self.memory.push(new_var);

        self.memory.last_mut().unwrap()
    }

    pub fn set_var(&mut self, var: &mut Variable, new_val: Literal) -> TranslitResult<()> {
        if var.0 != new_val {
            return Err(TranslitError::InvalidTypeError(Arg::from(new_val)));
        }
